...
% joint causal relationship (u(t),h_W(t),τ(t)) → (T_C,T_E,I) over time t∈[0,T], 
% as well as any of its conditional variants, i.e., where we resolve the 
% observational contributions of any subset of one or two of the observed 
% variables from this causal relationship through the conditional ACI framework;
% See Section 1.4 of the Supplementary Information. Therefore, in this script we 
% consider (T_C,T_E,I) to be the observables, while (u,h_W,τ) are the unobserved 
% variables. This code uses the same model parameter values as those cited in 
% the following paper: 
...
% 
%     L_x(:, :, j) = [
%         I(j)/5 * factor, gamma_C, 0.8 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor);
%         0, gamma_E, (1 + (1 - I(j)/5))*0.15 * sqrt(factor);
%         0, 0, 0
%     ];
%     f_x(:, j) = [
%         (r_C - (25 * (T_C(j) + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin((j)*dt*2*pi/6 - pi/6)) * factor) * T_C(j) + zeta_C * T_E(j) + C_u;
%         (r_E - 1.4 * factor * (1 + 0.3*sin((j)*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*(j)*dt*2*pi/6 + 2*pi/6))) * T_E(j) - zeta_E * T_C(j);
%         - lambda * (I(j) - m)
%     ];
%     S_x(:, :, j) = [
%         sigma_C, 0, 0;
...
%     ];
%     L_y(:, :, j) = [
%         -r, 0, -0.2 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor);
%         0, -r, -0.4 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor);
%         0, 0, -d_tau
%     ];
%     f_y(:, j) = [
%         - delta_u * (T_C(j) + T_E(j))/2;
...
%         sigma_u, 0, 0;
%         0, sigma_h, 0;
%         0, 0, 0.9 * (tanh(7.5*T_C(j)) + 1) * (1 + 0.3*cos(j*dt*2*pi/6 + 2*pi/6))
%     ];
% 
% end
% 
...
% colormap("jet")
% clim([-1.5, 1.5])
% cb = colorbar('eastoutside', Ticks=-1.5:0.5:1.5, TickLabels=arrayfun(@(x) sprintf('10^{%.1f}', x), -1.5:0.5:1.5, 'UniformOutput', false));
...
% IN THIS SCRIPT:
%   • Observables: x = (T_C, T_E, I)
%   • Unobserved variables: y = (u, h_W, τ)
...
L_x(:, :, 1) = [
    I(1)/5 * factor, gamma_C, 0.8 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor);
    0, gamma_E, (1 + (1 - I(1)/5))*0.15 * sqrt(factor);
    0, 0, 0
];
f_x(:, 1) = [
    (r_C - (25 * (T_C(1) + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(0*dt*2*pi/6 - pi/6)) * factor) * T_C(1) + zeta_C * T_E(1) + C_u;
    (r_E - 1.4 * factor * (1 + 0.3*sin(0*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*0*dt*2*pi/6 + 2*pi/6))) * T_E(1) - zeta_E * T_C(1);
    - lambda * (I(1) - m)
];
S_x(:, :, 1) = [
    sigma_C, 0, 0;
...
L_y(:, :, 1) = [
    -r, 0, -0.2 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor);
    0, -r, -0.4 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor);
    0, 0, -d_tau
];
f_y(:, 1) = [
    - delta_u * (T_C(1) + T_E(1))/2;
    - delta_h * (T_C(1) + T_E(1))/2;
    0
];
S_y(:, :, 1) = [
    sigma_u, 0, 0;
    0, sigma_h, 0;
    0, 0, 0.9 * (tanh(7.5*T_C(1)) + 1) * (1 + 0.3*cos(0*dt*2*pi/6 + 2*pi/6))
];
...
% Use the following to study (u(t),h_W(t),τ(t)) → T_C | (T_E,I) over time 
% t∈[0,T]:
%   S_xoS_x_inv(1, 1, :) = 1/sigma_C^2;
% Use the following to study (u(t),h_W(t),τ(t)) → T_E | (T_C,I) over time 
% t∈[0,T]:
%   S_xoS_x_inv(2, 2, :) = 1/sigma_E^2;
% Use the following to study (u(t),h_W(t),τ(t)) → I | (T_C,T_E) over time 
% t∈[0,T]:
...
E_j_matrices = zeros(3, 3, N+1);
F_j_matrices = zeros(3, 3, N+1);
G_y_j = L_y(:, :, N+1) + S_yoS_y(:, :, N+1) / filter_cov(:, :, N+1);
C_jj = eye(3) - G_y_j * dt;
...
for j = N:-1:1
    ...
    G_y_j = L_y(:, :, j) + S_yoS_y(:, :, j) / filter_cov(:, :, j);
    C_jj = eye(3) - G_y_j * dt;
    E_j_matrices(:, :, j) = C_jj;
    ...
    % INCLUDE THEM IN THE STATE ESTIMATION EQUATIONS IN THIS SCRIPT FOR 
    % SIMPLICITY.
    mu = muT - (L_y(:, :, j) * muT + f_y(:, j) - B_j / filter_cov(:, :, j) * (filter_mean(:, j) - muT)) * dt;
...
% Hovmöller diagrams for a period of 5*hov_len_years years starting from the the 
% model simulation year of choice (hov_year_start).
figure('WindowState', 'maximized');
...
% Time period (in years) for each of the Hovmöller diagram subplots.
hov_len_years = 20; 
hov_len_months = hov_len_years*12;

% Model simulation year from which to start plotting the Hovmöller diagrams.
% Valid choices for hov_year_start are: {0, 1, ..., 2*T/12-5*hov_len_years}.
hov_year_start = 250;
% Window used in the definition of the ENSO events = 12 months; See docstring
% in the beginning of this script for details.
defn_window = 12; 
total_loop = (hov_len_years-1)*12/defn_window;

for j = 1:5

    % Subplot plotting range in simulation months.
    range_months = 12*(hov_year_start+hov_len_years*(j-1)):hov_len_months+12*(hov_year_start+hov_len_years*(j-1));
    % Subplot plotting range in simulation years.
    range_years = linspace(hov_year_start+hov_len_years*(j-1), hov_len_years+hov_year_start+hov_len_years*(j-1), 12*hov_len_years+1);
    % Subplot plotting range in array indices.
    range_idxs = range_months(1)*k_dt:range_months(end)*k_dt;
    % Subplot's y-axis: Model simulation time.
    range_sim_years = range_idxs/k_dt/12;
    ...
    plot([180, 180], [range_years(1), range_years(end)], 'k--', Linewidth=2, DisplayName='Dateline');
    temp_tau = range_idxs(1:round(k_dt/3):end);
    % Plotting τ and I along the dateline (180°) roughly every 10 days for 
    % clarity and optimal plotting, since k_dt is the total number of indices 
    % which cover a single month.
    plot(180 + 10*tau(temp_tau), range_sim_years(1:round(k_dt/3):end), 'color', [0.2, 0.49, 0.2], Linewidth=1.5, DisplayName='WWB (τ)');
    plot(180 + 10*I(temp_tau), range_sim_years(1:round(k_dt/3):end), 'color', [0.72, 0.27, 1], Linewidth=1.5, DisplayName='Walker Cell (I)');

    % The colored boxes on the left vertical axis (ranging from September to 
    % February) indicate the type of ENSO event in boreal winter, which are 
    % based on the definitions described in this script's docstring.
    for k = 1:total_loop
        % Extreme EP El Niño (EEP EN): If we are to adopt the actual definition, 
        % i.e., the one noted in the docstring, then we should check for:
        %   max(T_E_monthly(range_months(1)-9+k*defn_window:range_months(1)+2+k*defn_window)) >= 2.5
        % Here we adopt a slightly different definition of extreme EP El Niño so
        % we can capture more significant EP EN events to look into.
        if mean(T_E_monthly(range_months(1)-1+k*defn_window:range_months(1)+1+k*defn_window)) > 1.5 && mean(T_E_monthly(range_months(1)-1+k*defn_window:range_months(1)+1+k*defn_window)) > mean(T_C_monthly(range_months(1)-1+k*defn_window:range_months(1)+1+k*defn_window))
            plot([120, 120], [range_years(1-4+k*defn_window), range_years(1+1+k*defn_window)], 'r', Linewidth=5, DisplayName='EEP EN') 
        % Moderate EP El Niño (MEP EN).
        elseif mean(T_E_monthly(range_months(1)-1+k*defn_window:range_months(1)+1+k*defn_window)) > 0.5 && mean(T_E_monthly(range_months(1)-1+k*defn_window:range_months(1)+1+k*defn_window)) > mean(T_C_monthly(range_months(1)-1+k*defn_window:range_months(1)+1+k*defn_window))
...
sgtitle('Hovmöller Diagram of Niño3.4 Using a Spatiotemporal Bivariate Linear Model of T_C & T_E');
cb = colorbar('eastoutside', Ticks=-3:3, TickLabels=arrayfun(@(x) sprintf('%d°C', x), -3:3, 'UniformOutput', false));
cb.Position(1) = cb.Position(1) + 0.1;
fontsize(16, 'points')
...
L_y_s = L_y(:, :, sim_month_start:sim_month_end);
f_y_s = f_y(:, sim_month_start:sim_month_end);
S_y_s = S_y(:, :, sim_month_start:sim_month_end);
...
signal_smoother_filter = zeros(1, length(filter_mean_s));
dispersion_smoother_filter = zeros(1, length(filter_mean_s));
ACI_metric = zeros(1, length(filter_mean_s));
for i = 1:length(filter_mean_s)
    signal_smoother_filter(i) = 0.5 * (smoother_mean_s(:, i) - filter_mean_s(:, i))' / filter_cov_s(:, :, i) * (smoother_mean_s(:, i) - filter_mean_s(:, i));
    cov_ratio = smoother_cov_s(:, :, i) / filter_cov_s(:, :, i);
    dispersion_smoother_filter(i) = 0.5 * (-log(abs(det(cov_ratio))) + trace(cov_ratio) - size(filter_mean_s, 1));
    ACI_metric(i) = signal_smoother_filter(i) + dispersion_smoother_filter(i);
end
...
for n = 1:(N_s-2)
    update_matrices_fixed{n} = zeros(3, 3, n+1);
    online_fixed_mean{n} = zeros(3, n);
    online_fixed_cov{n} = zeros(3, 3, n);
end
for n = N_s-1:N_s
    online_fixed_mean{n} = zeros(3, n);
    online_fixed_cov{n} = zeros(3, 3, n);
...
if fixed_lag == 0
    ...
    aux_vec = filter_mean_s(:, 1) ...
              - E_j_matrices_s(:, :, 1) * ((eye(3) + L_y_s(:, :, 1) * dt) * filter_mean_s(:, 1) + f_y_s(:, 1) * dt) ...
    ...
    aux_mat = filter_cov_s(:, :, 1) ...
              - E_j_matrices_s(:, :, 1) * (eye(3) + L_y_s(:, :, 1) * dt) * filter_cov_s(:, :, 1) ... 
...
for n = 3:N_s+1
    ...
    if fixed_lag == 0
        ...
        aux_vec = filter_mean_s(:, n-1) ...
                  - E_j_matrices_s(:, :, n-1) * ((eye(3) + L_y_s(:, :, n-1) * dt) * filter_mean_s(:, n-1) + f_y_s(:, n-1) * dt) ...
        ...
        aux_mat = filter_cov_s(:, :, n-1) ...
                  - E_j_matrices_s(:, :, n-1) * (eye(3) + L_y_s(:, :, n-1) * dt) * filter_cov_s(:, :, n-1) ... 
    ...
    for j = (n-1):-1:1
        ...
        elseif (n-fixed_lag <= j) && (j <= n-1)
            ...
            if j == n-1
                update_matrices_fixed{n-2}(:, :, n-1) = eye(3);
            elseif j == n-2
    ...
    for j = first_idx:last_idx
        ...
        if eps_idx == 1
            ...
            for obs = j:last_idx
                ...
                RE_n(obs-j+1) = 0.5 * (online_fixed_mean{end}(:, j) - online_fixed_mean{obs}(:, j))' / online_fixed_cov{obs}(:, :, j) * (online_fixed_mean{end}(:, j) - online_fixed_mean{obs}(:, j)) ...
                                + 0.5 * (-log(abs(det(cov_ratio))) + trace(cov_ratio) - size(filter_mean_s, 1));
            end
