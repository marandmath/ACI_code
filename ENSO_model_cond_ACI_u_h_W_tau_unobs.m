...
% variables. This code uses the same parameter values as those cited in the 
% following paper: 
...
for j = 1:5
    ...
    range_months = 1+12*(hov_year_start+hov_len_years*(j-1)):hov_len_months+12*(hov_year_start+hov_len_years*(j-1));
    ...
    % Plotting τ and I roughly every 10 days for clarity and optimal plotting, 
    % since k_dt is the total number of indices which cover a single month. 
    ...
    for k = 1:total_loop
        ...
        %   max(T_E_monthly(range_months(1)-9+k*window:range_months(1)+2+k*window)) >= 2.5
