...
% conditional causal relationship h_W(t) → (T_C,T_E,I) | (u,τ) over time 
...
% (u,T_C,T_E,τ,I) to be the observables, while h_W is the unobserved variable. 
...
% [fList, pList] = matlab.codetools.requiredFilesAndProducts('ENSO_model_cond_ACI_h_W_unobs.m');
%
% M-file Scripts:
...
% 
%     f_x(:, j) = [
%         (r_C - (25 * (T_C(j) + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(j*dt*2*pi/6 - pi/6)) * factor) * T_C(j) + zeta_C * T_E(j) + I(j)/5 * factor * u(j) + C_u + (0.8 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor)) * tau(j);
%         (r_E - 1.4 * factor * (1 + 0.3*sin(j*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*j*dt*2*pi/6 + 2*pi/6))) * T_E(j) - zeta_E * T_C(j) + ((1 + (1 - I(j)/5)) * 0.15 * sqrt(factor)) * tau(j);
%         - lambda * (I(j) - m)
%     ];
%     S_x(:, :, j) = [
%         sigma_C, 0, 0;
...
%     ];
%     f_y(j) = - delta_h * (T_C(j) + T_E(j)) / 2 + (-0.4 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor)) * tau(j);
% 
% end
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     plot([150*h_W(j), 150*h_W(j)], [j, j+approx_objective_CIR(j-time_start_plot+1)/dt], 'color', [0, 0, 0], Linewidth=2)
%     plot(150*h_W(j), j+approx_objective_CIR(j-time_start_plot+1)/dt, 'color', 'k', 'marker', '_', Linewidth=1)
% end
% ylim([time_start_plot, time_end_plot])
...
% yticks(time_start_plot:12*k_dt:time_end_plot)
% yticklabels(string((time_start_plot:12*k_dt:time_end_plot)/1200))
% legend('h_W', 'Objective CIR Length')
...
% IN THIS SCRIPT:
%   • Observables: x = (u,T_C,T_E,τ,I)
%   • Unobserved variables: y = h_W

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  NOTE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IN THE FOLLOWING, WE CAN EITHER CONSIDER u AND τ AS PART OF THE OBSERVABLES, 
...
% AND HAVE u AND τ TO BE DETERMINISTIC FORCINGS DEFINED BY THEIR OBSERVED VALUES 
% (IN THIS CASE THOSE GENERATED BY THE MODEL SIMULATION) DURING THE BAYESIAN 
% UPDATE WHEN DERIVING THE POSTERIOR FILTER AND SMOOTHER DISTRIBUTIONS; THIS 
% STRATEGY IS MADE SIMPLE BY THE FACT THERE IS ALSO NO NOISE CROSS-INTERACTING 
% TERMS TO CONSIDER FOR THIS MODEL. NOTE THAT BOTH APPROACHES, EITHER THIS ONE 
% OR THE ONE WITH x = (u,T_C,T_E,τ,I), WILL YIELD THE SAME RESULTS DURING 
% CONDITIONAL ACI ANALYSIS, BUT THE FORMER IS MUCH MORE COMPUTATIONALLY 
% EFFICIENT.
...
f_x(:, 1) = [
    (r_C - (25 * (T_C(1) + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(0*dt*2*pi/6 - pi/6)) * factor) * T_C(1) + zeta_C * T_E(1) + I(1)/5 * factor * u(1) + C_u + (0.8 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor)) * tau(1);
    (r_E - 1.4 * factor * (1 + 0.3*sin(0*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*0*dt*2*pi/6 + 2*pi/6))) * T_E(1) - zeta_E * T_C(1) + ((1 + (1 - I(1)/5)) * 0.15 * sqrt(factor)) * tau(1);
    - lambda * (I(1) - m)
];
S_x(:, :, 1) = [
    sigma_C, 0, 0;
...
f_y(1) = - delta_h * (T_C(1) + T_E(1)) / 2 + (-0.4 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor)) * tau(1);
...
for j = 2:N+1
    ...
    filter_cov(j) = filter_cov(j-1) ...
                    + (L_y * filter_cov(j-1) + filter_cov(j-1) * L_y' + S_yoS_y) * dt ...
...
if fixed_lag == 0
    ...
    aux_vec = filter_mean_s(1) ...
              ...
              + F_j_matrices_s(:, :, 1) * ([T_C_s(2)-T_C_s(1); T_E_s(2)-T_E_s(1); I_s(2)-I_s(1)] - (L_x * filter_mean_s(1) + f_x_s(:, 1)) * dt);
...
for n = 3:N_s+1
    ...
    if fixed_lag == 0
        ...
        aux_vec = filter_mean_s(n-1) ...
                  ...
                  + F_j_matrices_s(:, :, n-1) * ([T_C_s(n)-T_C_s(n-1); T_E_s(n)-T_E_s(n-1); I_s(n)-I_s(n-1)] - (L_x * filter_mean_s(n-1) + f_x_s(:, n-1)) * dt);
