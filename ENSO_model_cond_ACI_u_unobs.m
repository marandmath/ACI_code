...
% conditional causal relationship u(t) → (T_C,T_E,I) | (h_W,τ) over time 
...
% (h_W,T_C,T_E,τ,I) to be the observables, while u is the unobserved variable. 
...
% [fList, pList] = matlab.codetools.requiredFilesAndProducts('ENSO_model_cond_ACI_u_unobs.m');
%
% M-file Scripts:
...
% for j = 1:N+1
% 
%     L_x(:, j) = [I(j)/5 * factor; 0; 0];
%     f_x(:, j) = [
%         (r_C - (25 * (T_C(j) + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(j*dt*2*pi/6 - pi/6)) * factor) * T_C(j) + zeta_C * T_E(j) + gamma_C * h_W(j) + C_u + (0.8 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor)) * tau(j);
%         (r_E - 1.4 * factor * (1 + 0.3*sin(j*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*j*dt*2*pi/6 + 2*pi/6))) * T_E(j) - zeta_E * T_C(j) + gamma_E * h_W(j) + ((1 + (1 - I(j)/5)) * 0.15 * sqrt(factor)) * tau(j);
%         - lambda * (I(j) - m)
%     ];
%     S_x(:, :, j) = [
%         sigma_C, 0, 0;
...
%     ];
%     f_y(j) = - delta_u * (T_C(j) + T_E(j)) / 2 + (-0.2 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor)) * tau(j);
% 
% end
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% time_start_plot = ((sim_year_start-obs_start_year)+2)*12*k_dt;
% time_end_plot = ((sim_year_start-obs_start_year)+ACI_period_years-2)*12*k_dt;
% plot(1.5 * u(time_start_plot:time_end_plot), time_start_plot:time_end_plot, 'm', Linewidth=2)
% hold on
...
%     plot([1.5*u(j), 1.5*u(j)], [j, j+approx_objective_CIR(j-time_start_plot+1)/dt], 'color', [0, 0, 0], Linewidth=2)
%     plot(1.5*u(j), j+approx_objective_CIR(j-time_start_plot+1)/dt, 'color', 'k', 'marker', '_', Linewidth=1)
% end
% ylim([time_start_plot, time_end_plot])
...
% IN THIS SCRIPT:
%   • Observables: x = (h_W,T_C,T_E,τ,I)
%   • Unobserved variables: y = u

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  NOTE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IN THE FOLLOWING, WE CAN EITHER CONSIDER h_W AND τ AS PART OF THE OBSERVABLES, 
...
% AND HAVE h_W AND τ TO BE DETERMINISTIC FORCINGS DEFINED BY THEIR OBSERVED 
...
% EITHER THIS ONE OR THE ONE WITH x = (h_W,T_C,T_E,τ,I), WILL YIELD THE SAME 
...
L_x(:, 1) = [I(1)/5 * factor; 0; 0];
f_x(:, 1) = [
    (r_C - (25 * (T_C(1) + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(0*dt*2*pi/6 - pi/6)) * factor) * T_C(1) + zeta_C * T_E(1) + gamma_C * h_W(1) + C_u + (0.8 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor)) * tau(1);
    (r_E - 1.4 * factor * (1 + 0.3*sin(0*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*0*dt*2*pi/6 + 2*pi/6))) * T_E(1) - zeta_E * T_C(1) + gamma_E * h_W(1) + ((1 + (1 - I(1)/5))*0.15 * sqrt(factor)) * tau(1);
    - lambda * (I(1) - m)
];
S_x(:, :, 1) = [
    sigma_C, 0, 0;
...
f_y(1) = - delta_u * (T_C(1) + T_E(1)) / 2 + (-0.2 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor)) * tau(1);
...
% Use the following to study u(t) → T_C | (h_W,T_E,τ,I) over time t∈[0,T]:
S_xoS_x_inv(1, 1, :) = 1/sigma_C^2;
% Use the following to study u(t) → T_E | (h_W,T_C,τ,I) over time t∈[0,T]:
%   S_xoS_x_inv(2, 2, :) = 1/sigma_E^2;
% Use the following to study u(t) → I | (h_W,T_C,T_E,τ) over time t∈[0,T]:
