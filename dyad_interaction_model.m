...
% while y is the unobserved variable. This code uses the same parameter values 
...
cb.Position(3) = 0.015;
ylim([lowest_order/2, highest_order])
set(gca, 'YDir','reverse')
title('Subjective CIR Length for y(t) \rightarrow x (Logarithmic Scale)')
xlabel('t')
ylabel('ε')
yticks(round(lowest_order/2):highest_order)
yticklabels(arrayfun(@(x) sprintf('10^{%d}', x), round(lowest_order/2):highest_order, 'UniformOutput', false))
fontsize(16, 'points')
