...
% conditional causal relationship T_C(t) → (T_E,I) | (h_W,u,τ) over time 
% t∈[0,T], as well as any of its other conditional variants, i.e., where we
% additionally resolve the observational contributions of any the two observed 
% variables (T_E,I) from this conditional causal relationship through the 
% conditional ACI framework; See Section 1.4 of the Supplementary Information. 
% Therefore, in this script we consider (u,h_W,T_E,τ,I) to be the observables, 
% while T_C is the unobserved variable. For this to be possible, we implement a
% zeroth-order accurate Taylor approximation of c_1(t,T_C) around the 
% climatology T_C = 0, thus transforming the feedback of T_C in T_C to be state 
% independent, r_C-c_1(t,0), which achieves conditional linearity in T_C. This 
% code uses the same parameter values as those cited in the following paper: 
...
% IN THIS SCRIPT:
%   • Observables: x = (u,h_W,T_E,τ,I)
%   • Unobserved variables: y = T_C

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  NOTE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% IN THE FOLLOWING, WE CAN EITHER CONSIDER u, h_W, AND τ AS PART OF THE 
% OBSERVABLES, AS ALREADY OUTLINED, OR WE CAN INSTEAD APPLY CONDITIONAL ACI BY 
% PROXY VIA AN OPERATIONAL SHORTCUT, AS DESCRIBED IN SECTION 2.3 OF THE MAIN 
% TEXT:
...
%    time series[.]"
% THEREFORE, IN THE FOLLOWING IT IS EQUIVALENT TO SIMPLY ASSUME x = (T_E,I) AND 
% HAVE u, h_W, AND τ TO BE DETERMINISTIC FORCINGS DEFINED BY THEIR OBSERVED 
...
% EITHER THIS ONE OR THE ONE WITH x = (u,h_W,T_E,τ,I), WILL YIELD THE SAME 
...
f_x(:, 1) = [
    (r_E - 1.4 * factor * (1 + 0.3*sin(0*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*0*dt*2*pi/6 + 2*pi/6))) * T_E(1) + gamma_E * h_W(1) + ((1 + (1 - I(1)/5)) * 0.15 * sqrt(factor)) * tau(1);
    - lambda * (I(1) - m)
];
S_x(:, :, 1) = [
    sigma_E, 0;
    0, sqrt(lambda * (4-I(1)) * I(1))
];

% NOTICE HOW WE SUBSTITUTE c_1(t,T_C) WITH c_1(t,0), AS OUTLINED IN THE
% DOCSTRING OF THIS SCRIPT.
L_y(1) = (r_C - (25 * (0 + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(0*dt*2*pi/6 - pi/6)) * factor);
f_y(1) = zeta_C * T_E(1) + I(1)/5 * factor * u(1) + C_u + (0.8 * (1 + (1 - I(1)/5))*0.15 * sqrt(factor)) * tau(1);
...
for j = 2:N+1
    ...
    % deviation/discharge coefficient; NOTE HOW THE MODEL SIMULATED DATA, WHICH 
    % CONSTITUTE THE OBSERVATIONS IN THIS SCRIPT, ARE STILL GENERATED USING THE 
    % TRUE FORM OF THE DAMPING c_1(t,T_C).
    ...
    f_x(:, j) = [
        (r_E - 1.4 * factor * (1 + 0.3*sin(j*dt*2*pi/6 + 2*pi/6) + 0.25*sin(2*j*dt*2*pi/6 + 2*pi/6))) * T_E(j) + gamma_E * h_W(j) + ((1 + (1 - I(j)/5)) * 0.15 * sqrt(factor)) * tau(j);
        - lambda * (I(j) - m)
    ];
    S_x(:, :, j) = [
        sigma_E, 0;
        0, sqrt(lambda * (4-I(j)) * I(j))
    ];
    
    % NOTICE HOW WE SUBSTITUTE c_1(t,T_C) WITH c_1(t,0), AS OUTLINED IN THE
    % DOCSTRING OF THIS SCRIPT.
    L_y(j) = (r_C - (25 * (0 + 0.75/7.5).^2 + 0.9) * (1 + 0.3*sin(j*dt*2*pi/6 - pi/6)) * factor);
    f_y(j) = zeta_C * T_E(j) + I(j)/5 * factor * u(j) + C_u + (0.8 * (1 + (1 - I(j)/5))*0.15 * sqrt(factor)) * tau(j);
...
% Use the following to study T_C(t) → T_E | (u,h_W,τ,I) over time t∈[0,T]:
S_xoS_x_inv(1, 1, :) = 1/sigma_E^2;
% Use the following to study T_C(t) → I | (u,h_W,T_E,τ) over time t∈[0,T]:
%   sigma_I = sqrt(lambda .* (4-I) .* I);
%   S_xoS_x_inv(2, 2, :) = 1./sigma_I.^2;
%   S_xoS_x_inv(2, 2, sigma_I == 0) = 0;
...
for j = 2:N+1
    ...
    filter_mean(j) = filter_mean(j-1) ...
                     ...
                     + filter_cov(j-1) * L_x' * S_xoS_x_inv(:, :, j-1) * (dx - (L_x * filter_mean(j-1) + f_x(:, j-1)) * dt);
    filter_cov(j) = filter_cov(j-1) ...
                    + (L_y(j-1) * filter_cov(j-1) + filter_cov(j-1) * L_y(j-1)' + S_yoS_y) * dt ...
                    - (filter_cov(j-1) * L_x' * S_xoS_x_inv(:, :, j-1) * L_x * filter_cov(j-1)) * dt;
...
F_j_matrices = zeros(1, 2, N+1);
G_y_j = L_y(N+1) + S_yoS_y / filter_cov(N+1);
C_jj = 1 - G_y_j * dt;
E_j_matrices(N+1) = C_jj;
F_j_matrices(:, :, N+1) = G_y_j * filter_cov(N+1) * L_x' * S_xoS_x_inv(:, :, N+1) * dt;
...
for j = N:-1:1
    ...
    C_jj = 1 - G_y_j * dt;
    E_j_matrices(j) = C_jj;
    F_j_matrices(:, :, j) = G_y_j * filter_cov(j) * L_x' * S_xoS_x_inv(:, :, j) * dt;
...
if fixed_lag == 0
    ...
    aux_vec = filter_mean_s(1) ...
              - E_j_matrices_s(1) * ((1 + L_y(1) * dt) * filter_mean_s(1) + f_y_s(1) * dt) ...
              + F_j_matrices_s(:, :, 1) * ([T_E_s(2)-T_E_s(1); I_s(2)-I_s(1)] - (L_x * filter_mean_s(1) + f_x_s(:, 1)) * dt);
    online_fixed_mean{2}(1) = E_j_matrices_s(1) * filter_mean_s(2) + aux_vec;
    aux_mat = filter_cov_s(1) ...
              - E_j_matrices_s(1) * (1 + L_y(1) * dt) * filter_cov_s(1) ... 
              - F_j_matrices_s(:, :, 1) * L_x * filter_cov_s(1) * dt;
...
for n = 3:N_s+1
    ...
    if fixed_lag == 0
        ...
        aux_vec = filter_mean_s(n-1) ...
                  - E_j_matrices_s(n-1) * ((1 + L_y(n-1) * dt) * filter_mean_s(n-1) + f_y_s(n-1) * dt) ...
                  + F_j_matrices_s(:, :, n-1) * ([T_E_s(n)-T_E_s(n-1); I_s(n)-I_s(n-1)] - (L_x * filter_mean_s(n-1) + f_x_s(:, n-1)) * dt);
        online_fixed_mean{n}(n-1) = E_j_matrices_s(n-1) * filter_mean_s(n) + aux_vec;
        aux_mat = filter_cov_s(n-1) ...
                  - E_j_matrices_s(n-1) * (1 + L_y(n-1) * dt) * filter_cov_s(n-1) ... 
                  - F_j_matrices_s(:, :, n-1) * L_x * filter_cov_s(n-1) * dt;
...
for j = time_start_plot:k_dt:time_end_plot
    plot([7.5*T_C(j), 7.5*T_C(j)], [j, j+approx_objective_CIR(j-time_start_plot+1)/dt], 'color', [0, 0, 0], Linewidth=2)
    plot(7.5*T_C(j), j+approx_objective_CIR(j-time_start_plot+1)/dt, 'color', 'k', 'marker', '_', Linewidth=1)
end
ylim([time_start_plot, time_end_plot])
...
yticks(time_start_plot:12*k_dt:time_end_plot)
yticklabels(string((time_start_plot:12*k_dt:time_end_plot)/1200))
legend('T_C', 'Objective CIR Length')
...
colormap("jet")
clim([-1, 1])
cb = colorbar('eastoutside', Ticks=-1:0.5:1, TickLabels=arrayfun(@(x) sprintf('10^{%.1f}', x), -1:0.5:1, 'UniformOutput', false));
